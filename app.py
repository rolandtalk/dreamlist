...
def calculate_performance_and_rsi(symbol, session=None):
    ...
    # Use live price from meta.regularMarketPrice when available;
    # it reflects the latest traded price (intraday or after-hours),
    # so perf_1d always shows change vs the previous close.
    ...
    if live_price is not None and abs(c_now - last_bar) > 0.001:
        # Live price differs from last daily bar: the bar list doesn't
        # include today yet.  Use second-to-last bar as "previous close".
